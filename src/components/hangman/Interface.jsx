    ...
    <>
      {phase === "ready" && (
        <div className="fixed z-10 text-[#00171F] top-0 left-0 w-full p-4 flex flex-col gap-4">
          <form
            className="flex flex-col gap-4 max-w-sm bg-white p-4 rounded-xl"
      ...
      {(phase === "playing" || phase === "ended" || phase === "won") && (
        <div className="fixed z-10 text-[#00171F] top-0 left-0 w-full p-4">
          <div className="flex flex-col gap-4 items-center sm:justify-between sm:flex-row">
            <p className="text-2xl uppercase sm:text-3xl">{category}</p>
            <div className="flex gap-2">
...
function renderStrikes(count, maxCount) {
  ...
  for (let i = 0; i < maxCount; i++) {
    blocks.push(
      <div 
        key={i} 
        className={`rounded-sm aspect-square w-8 md:w-10 bg-white ${
...
function createSpaces(word, guesses) {
  return word.split("").map((letter, index) => (
    <div
      ...
      key={`${letter}-${index}`}
      className="aspect-square w-6 p-2 border-b-2 border-[#00171F] flex items-center justify-center text-2xl mt-4 sm:mt-2 sm:w-14 sm:text-3xl"
    >
